...
from Shomate import N2, H2, NH3, K_std
...
class EqNet(pl.LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        p_norm = X[:, 1]
        p = (
            p_norm * self.trainer.datamodule.X_std[1]
            + self.trainer.datamodule.X_mean[1]
        ) * 1e5

        T_norm = X[:, 0]
        T = (
            T_norm * self.trainer.datamodule.X_std[0]
            + self.trainer.datamodule.X_mean[0]
        )
        ...
        x_H2, x_NH3 = y_pred[:, 0], y_pred[:, 1]
        x_N2 = torch.ones_like(x_H2) - x_H2 - x_NH3
        MWG = (p * x_H2 / 1e5) ** -3 * (p * x_N2 / 1e5) ** -1 * (p * x_NH3 / 1e5) ** 2
        K = torch.empty_like(MWG)
        for i, T_i in enumerate(T):
            K[i] = K_std(T_i.cpu(), [-1, -3, 2], [N2, H2, NH3])

        print(MWG)
        print(K)

        # MWG_loss = torch.nn.functional.mse_loss(MWG, K)
        MWG_loss = tm.functional.mean_absolute_percentage_error(MWG, K)
